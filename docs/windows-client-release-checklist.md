...
This is the clean-machine and installed-client pass. It preserves the release gates, live smoke results, packaging checks, tray/autostart behavior, sign-in flow, sync-pair validation, diagnostics, update, uninstall, and CLI smoke coverage without committing evidence files.
...
- Manual installed-release GUI and reboot/logon smoke closed the release checklist on this host. desktop automation is still unavailable in this desktop automation session: bootstrap fails with `Package subpath './dist/project/cua/sky_js/src/targets/windows/internal/computer_use_client_base.js' is not defined by "exports" in <local desktop automation runtime path>`. Future fully automated tray/logon coverage needs a working desktop automation helper or a dedicated Windows UI harness, but this is no longer a release blocker for this smoke checklist.
