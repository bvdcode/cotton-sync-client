    ...
    public class DesktopPackagingMetadataTests
    {
        ...
        public void LinuxGuiScreenshotMatrixScript_CapturesDefaultVisualSmokeStates()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(smokeScript, Does.Contain("DISPLAY is required"));
                Assert.That(smokeScript, Does.Contain("set -- connecting sign-in-error empty-dashboard add-folder dashboard folder-controls progress settings settings-diagnostics error conflict"));
